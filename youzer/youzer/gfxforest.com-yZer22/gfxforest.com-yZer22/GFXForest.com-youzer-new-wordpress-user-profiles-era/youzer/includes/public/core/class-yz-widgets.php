...
class Youzer_Widgets {
	...
	function check_widget_content2( $widget_name, $data ) {

		$meta_options = array(
			'portfolio', 'services', 'skills', 'quote', 'link', 'video', 'post', 'about_me', 'project'
		);

		if ( in_array( $widget_name, $this->infos_boxes ) ) {
			if ( ! yz_get_xprofile_field_value( $data ) ) {
				return false;
			}
		} elseif ( in_array( $widget_name, $meta_options )  ) {
			if ( ! yz_data( $data ) ) {
				return false;
			}
	...
	function widgets_without_settings() {
		$widgets = array(
			'friends', 'groups', 'social_networks', 'recent_posts', 'phone', 'address', 'email', 'website', 'user_tags', 'user_balance', 'user_badges', 'reviews', 'basic_infos', 'custom_widgets'
		);
