...
class Youzer_Reviews {
	...
	function handle_user_reviews() {
		...
		if ( $action == 'add' ) {
			...
			if ( $data['reviewed'] == $data['reviewer'] ) {
				$response['error'] = __( "sorry you can't post a review to yourself.", 'youzer' );
				die( json_encode( $response ) );
			}
	...
	function delete_user_review() {
		...
		if ( ! $review_data ) {
			$response['error'] = __( 'The review is already deleted or not exist.', 'youzer' );
			die( json_encode( $response ) );
		}
	...
	function scripts() {
	    ...
	    // Call Review Script.
	    wp_enqueue_style( 'yz-reviews', YZ_PA . 'css/yz-reviews.min.css' );

	    // Call Bookmark Posts Script.
	    wp_enqueue_script( 'yz-reviews', YZ_PA . 'js/yz-reviews.min.js', array( 'jquery' ), $Youzer->version, true );
	    ...
	    if ( bp_is_current_component( $reviews_slug ) ) {
	        wp_enqueue_script( 'yz-reviews-pagination', YZ_PA . 'js/yz-reviews-pagination.min.js' );
	    }
