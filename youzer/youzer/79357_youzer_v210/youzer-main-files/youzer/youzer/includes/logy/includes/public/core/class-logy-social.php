...
class Logy_Social {
	...
	public function create_new_user( $user ) {
		...
		// Get Unique Username.
		$user_login = $this->get_unique_username( $display_name, $user->username );
	 ...
	 */
	public function bp_register_user( $user, $user_meta = null ) {
		...
		$user_password = wp_generate_password();
	 	...
	 	// Add New User.
		$user_id = bp_core_signup_user( $user->displayName, $user_password, $user->email, $usermeta );
		...
		// Get Provider
		$provider = ! empty( $get_session ) ? $get_session : $this->get_provider();
    ...
    function get_inactive_account_message( $user_login = null ) {
    	...
    	// Get Inactive Account Message.
		$inactive_account_msg = __( 'Your account has not been activated. Check your email for the activation link.', 'youzer' );
		...
		$resend_string = '<br />' . sprintf( __( 'If you have not received an email yet, <a href="%s">click here to resend it</a>.', 'youzer' ), esc_url( $resend_url ) );
