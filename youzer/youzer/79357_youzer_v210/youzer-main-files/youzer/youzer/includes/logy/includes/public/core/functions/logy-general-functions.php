...
function logy_add_message( $message, $type = null ) {
    ...
    @setcookie( 'logy-message', $message, time() + 60 * 60 * 24, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
    @setcookie( 'logy-message-type', $type, time() + 60 * 60 * 24, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
...
function logy_core_setup_message() {
    ...
    if ( empty( $Logy->template_message ) && isset( $_COOKIE['logy-message'] ) ) {
        $Logy->template_message = stripslashes( $_COOKIE['logy-message'] );
    }

    if ( empty( $Logy->template_message_type ) && isset( $_COOKIE['logy-message-type'] ) ) {
        $Logy->template_message_type = stripslashes( $_COOKIE['logy-message-type'] );
    }

    add_action( 'logy_form_notices', 'logy_core_render_message' );

    if ( isset( $_COOKIE['logy-message'] ) ) {
        @setcookie( 'logy-message', false, time() - 1000, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
    }

    if ( isset( $_COOKIE['logy-message-type'] ) ) {
        @setcookie( 'logy-message-type', false, time() - 1000, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
    }
}
add_action( 'init', 'logy_core_setup_message', 5 );
