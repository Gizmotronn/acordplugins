...
class Logy_Complete_Registration {
	...
	public function register_user() {
		...
		$user_profile_data = logy_user_profile_data( 'get' );
	 	...
	 	if ( empty( $user ) ) {
	        // Display Error.
			$errors[] = logy_get_message( $this->logy->login->get_error_message( 'cant_connect' ) );
	 	}
	 	...
	 	$user->displayName = isset( $_POST['signup_username'] ) ? sanitize_user( $_POST['signup_username'] ) : $this->logy->social->get_unique_username( $user->displayName, $user->username );
		...
		if ( ! empty( $errors ) ) {
			...
			bp_core_redirect( logy_page_url( 'complete-registration' ) );
		}
