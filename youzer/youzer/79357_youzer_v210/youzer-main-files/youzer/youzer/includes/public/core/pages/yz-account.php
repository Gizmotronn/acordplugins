...
class Youzer_Account {
	...
	function account_settings_pages() {
		...
		// Filter
		$pages = apply_filters( 'account_settings_pages', $pages );
	...
	function profile_settings_pages() {
		...
		// Filter
		$pages = apply_filters( 'profile_settings_pages', $pages );
	...
	function get_menu( $args ) {
		...
		if ( 'widgets-settings' == bp_current_component() ) {
			$current_widget = $menu[0]['widget_name'];
	        ...
	        // Get Widget Name.
	        $current_widget = $group_data[0]->name;
	    ...
	    echo "<h2>{$args['menu_title']}</h2><i class='fa fa-caret-up' aria-hidden='true'></i>";
	    echo "</div><ul>";
			...
			<ul>
				...
				<li>
					<a href="<?php echo esc_url( $page['href'] ); ?>" class="<?php echo $page['class'] ?>">
						<div class="yza-icon"><i class="fa fa-<?php echo $page['icon'] ?>" aria-hidden="true"></i></div>
