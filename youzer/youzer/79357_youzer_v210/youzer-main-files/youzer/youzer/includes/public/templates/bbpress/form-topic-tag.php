...
<?php if ( current_user_can( 'edit_topic_tags' ) ) : ?>
	...
	<div id="edit-topic-tag-<?php bbp_topic_tag_id(); ?>" class="bbp-topic-tag-form">
		...
		<div class="bbp-form yz-bbp-box" id="bbp-edit-topic-tag">
			...
			<fieldset class="bbp-form" id="tag-rename">
				...
				<form id="rename_tag" name="rename_tag" method="post" action="<?php the_permalink(); ?>">
					...
					<div class="bbp-submit-wrapper">
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" class="button submit"><i class="fa fa-refresh" aria-hidden="true"></i><?php esc_attr_e( 'Update', 'bbpress' ); ?></button>
			...
			<fieldset class="bbp-form" id="tag-merge">
				...
				<form id="merge_tag" name="merge_tag" method="post" action="<?php the_permalink(); ?>">
					...
					<div class="bbp-submit-wrapper">
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" class="button submit" onclick="return confirm('<?php echo esc_js( sprintf( __( 'Are you sure you want to merge the "%s" tag into the tag you specified?', 'bbpress' ), bbp_get_topic_tag_name() ) ); ?>');"><i class="fa fa-random" aria-hidden="true"></i><?php esc_attr_e( 'Merge', 'bbpress' ); ?></button>
			...
			<?php if ( current_user_can( 'delete_topic_tags' ) ) : ?>
				...
				<fieldset class="bbp-form" id="delete-tag">
					...
					<form id="delete_tag" name="delete_tag" method="post" action="<?php the_permalink(); ?>">
						...
						<div class="bbp-submit-wrapper">
							<button type="submit" tabindex="<?php bbp_tab_index(); ?>" class="button submit" onclick="return confirm('<?php echo esc_js( sprintf( __( 'Are you sure you want to delete the "%s" tag? This is permanent and cannot be undone.', 'bbpress' ), bbp_get_topic_tag_name() ) ); ?>');"><i class="fa fa-trash" aria-hidden="true"></i><?php esc_attr_e( 'Delete', 'bbpress' ); ?></button>
