...
function yz_get_file_url( $file ) {
	...
	if ( empty( $file_name ) ) {
		$file_name = isset( $file['original'] ) ? $file['original'] : $file['file_name'];
	}
...
function yz_save_image_thumbnail( $file, $activity_id = null ) {
    ...
    // Get Filename.
    $filename = isset( $file['original'] ) ? $file['original'] : $file['file_name'];
 			...
 			<div class="yz-embed-head">
	 			...
	 			<div class="yz-embed-meta">
	 				...
	 				<div class="yz-embed-meta-item">
	 					<i class="fa fa-users" aria-hidden="true"></i><span><?php echo sprintf( _n( '%s member', '%s members', $members_count, 'youzer' ), bp_core_number_format( $members_count ) ); ?></span>
	 				</div>
	 			</div>
 			</div>
 			<div class="yz-embed-action">
...
function yz_get_wall_new_post_thumb( $post_id = false ) {
    ...
    if ( ! empty( $img_url[0] ) ) {
        ...
        $thumbnail .= '<div class="yz-thumbnail-icon"><i class="fa fa-' . yz_get_format_icon( $format ) . '"></i></div>';
        $thumbnail .= '</div>';
 		...
 		<div class="yz-post-inner">
	 		...
	 		<div class="yz-post-head">
	 			...
	 			<div class="yz-post-meta">
	 				...
	 				<div class="yz-meta-item"><i class="fa fa-tags"></i><?php echo $categories; ?></div>
	 				<?php endif; ?>
	 				<div class="yz-meta-item"><i class="fa fa-calendar-o"></i><?php echo get_the_date( 'F j, Y', $post_id ); ?></div>
	 				<div class="yz-meta-item"><i class="fa fa-comments-o"></i><?php echo $post->comment_count; ?></div>
	 			</div>
	 		</div>
	 		<div class="yz-post-excerpt">
		        <p><?php echo yz_get_excerpt( $post->post_content, 40 ); ?></p>
	 		</div>
	 		<a href="<?php echo $post_link; ?>" class="yz-post-more-button"><span class="yz-btn-icon"><i class="fa fa-angle-double-right" aria-hidden="true"></i></span><span class="yz-btn-title"><?php _e( 'read more', 'youzer' ); ?></span></a>
 		</div>
 	</div>
...
function yz_get_open_graph_tags( $type = null, $url = null,  $title = nul, $description = null, $image = null ) {
    ...
    <?php if ( ! empty( $image ) ) : ?>
        <?php $image_size = getimagesize( $image ); ?>
