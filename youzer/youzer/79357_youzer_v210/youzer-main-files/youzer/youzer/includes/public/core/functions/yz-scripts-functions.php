...
function yz_global_scripts() {
    ...
    // Font Awesome.
    wp_register_style( 'yz-icons', YZ_AA . 'fonts/css/font-awesome.min.css', $Youzer->version );
...
function yz_public_scripts() {
    ...
    // Register Panel Style.
    wp_register_style( 'yz-emoji',  YZ_PA . 'css/emojionearea.min.css', $Youzer->version );
    ...
    // Emojionearea Script
    wp_register_script( 'yz-emojionearea', YZ_PA . 'js/emojionearea.min.js', $jquery, $Youzer->version, true );

    // Emoji Script
    wp_register_script( 'yz-emoji', YZ_PA . 'js/yz-emoji.min.js', $jquery, $Youzer->version, true );
    ...
    // Headers Css
    wp_register_style( 'yz-style', YZ_PA . 'css/youzer.min.css', array( 'yz-opensans', 'yz-icons' ), $Youzer->version );
    ...
    if ( yz_is_user_can_pin_posts() && (  bp_is_user_activity() || bp_is_groups_component() || bp_is_activity_directory() ) ) {

        // Call Sticky Posts Script.
        wp_enqueue_script( 'yz-sticky-posts', YZ_PA . 'js/yz-sticky-posts.min.js', $jquery, $Youzer->version, true );
    ...
    if ( yz_is_activity_component() ) {
        ...
        if ( 'on' == $enable_url_preview ) {
            // URL Preview CSS
            wp_enqueue_script( 'yz-angular', YZ_PA . 'js/angular.min.js', array('jquery', 'yz-emojionearea'), $Youzer->version );
            wp_enqueue_script( 'yz-url-preview', YZ_PA . 'js/yz-url-preview.js', array( 'jquery', 'yz-emojionearea' ), $Youzer->version );
            wp_enqueue_style( 'yz-url-preview', YZ_PA . 'css/yz-url-preview.min.css', $Youzer->version );
...
function yz_emoji_scripts() {
    ...
    // Call Emoji JS/CSS.
    wp_enqueue_style( 'yz-emoji' );
    wp_enqueue_script( 'yz-emojionearea' );
    wp_enqueue_script( 'yz-emoji' );
...
add_action( 'wp_enqueue_scripts', 'yz_emoji_scripts' );
