...
class Youzer_Setup {
    ...
    function install_new_version_options() {
        ...
        // Install Youzer 2.0.1 Options.
        if ( ! get_option( 'install_youzer_2.0.1_options' ) ) {
            update_option( 'yz_profile_default_tab', 'overview' );
            update_option( 'install_youzer_2.0.1_options', 1 );
        }

        if ( get_option( 'install_youzer_2.0.7_options' ) ) {
            return false;
        }
        ...
        foreach ( $new_widgets as $widget ) {
            if ( ! in_array( array( key( $widget ) => 'visible' ), $sidebar_widgets )
                &&
                 ! in_array( array( key( $widget ) => 'invisible' ), $sidebar_widgets )
            ) {
        ...
        // Add Custom Widgets ID.
		if ( ! get_option( 'yz_next_user_tag_nbr' ) ) {
			update_option( 'yz_next_user_tag_nbr', '1' );
		}

        // Hide Buddypress Toolbar 
        update_option( 'hide-loggedout-adminbar', 1 );

        // Mark New Options As Installed
        update_option( 'install_youzer_2.0.7_options', 1 );
