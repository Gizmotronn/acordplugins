 ...
 */
function yz_deregister_bbpress_styles() {
...
add_action( 'wp_print_styles', 'yz_deregister_bbpress_styles', 15 );
 ...
 */
function yz_bp_maybe_replace_template( $templates, $slug, $name ) {
     
    if ( 'members/single/home' == $slug || 'activity/single/home' == $slug  ) {
        if ( yz_is_account_page() ) {
            return array( 'account-template.php' );
        } else {
            return array( 'profile-template.php' );
        }
    }
...
function yz_bp_overload_templates() {
    ...
    if ( bp_is_user()  ) {
        add_filter( 'bp_get_template_part', 'yz_bp_maybe_replace_template', 10, 3 );
    }
 ...
 */
function yz_add_user_online_status_icon( $username = null ) {
