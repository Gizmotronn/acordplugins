...
class Youzer_User {
	...
	function location( $only_data = false, $user_id = null ) {
		...
		// Get Location HTML.
		$location = '<li><i class="fa fa-map-marker" aria-hidden="true"></i><span>' . sanitize_text_field( $user_location ) . '</span></li>';
	...
	function website() {
		...
		// Get Website HTML.
		$website = '<li><a href="' . esc_url( $user_website ) . '"><i class="fa fa-chain-broken" aria-hidden="true"></i><span>' . $user_website . '</span></a></li>';
	...
	function networks( $args = null ) {
		...
		foreach ( $social_networks as $network => $data ) {
			...
			if ( $link && $icon ) {
				echo "<li class='$network'><a href='$link' target='_blank'>";
	...
	function statistics( $args = null ) {
		...
		if ( yz_is_404_profile() ) {
			...
			// Get Data.
			$views_number 	 = $this->views( $user_id );
			$posts_number 	 = yz_get_user_posts_nbr( $user_id );
			$comments_number = yz_get_comments_number( $user_id );

			// Show/Hide Elements.
			$display_views 	  = yz_options( 'yz_display_' . $target . '_views' );
			$display_posts 	  = yz_options( 'yz_display_' . $target . '_posts' );
			$display_comments = yz_options( 'yz_display_' . $target . '_comments' );
		}
		
		if ( 'on' != $display_views && 'on' != $display_posts && 'on' != $display_comments ) {
			return false;
		}
			...
			<div class="<?php echo yz_generate_class( $statistics_class ); ?>">
				<ul>
					...
					<?php if ( 'on' == $display_comments ) : ?>
						<li>
							<div class="yz-snumber" title="<?php echo $comments_number; ?>"><?php echo $this->get_statistic_number( $comments_number ); ?></div>
							<h3 class="yz-sdescription"><?php _e( 'comments', 'youzer' ); ?></h3>
						</li>
					<?php endif; ?>

					<?php if ( 'on' == $display_views ) : ?>
						<li>
							<div class="yz-snumber" title="<?php echo $views_number; ?>"><?php echo $this->get_statistic_number( $views_number); ?></div>
							<h3 class="yz-sdescription"><?php _e( 'Views', 'youzer' ); ?></h3>
						</li>
					<?php endif; ?>
		...
		<div class="yz-settings-menu">
			<?php foreach ( $links as $link ) : ?>
				<a href="<?php echo esc_url( $link['href'] ); ?>">
					<div class="yz-icon"><i class="fa fa-<?php echo $link['icon'];?>" aria-hidden="true"></i></div>
	...
	function box_head( $target, $user_id = null ) {
		...
		?>
			<div class="yzb-head-content">
				<div class="yzb-user-status"><?php echo yz_add_user_online_status_icon(); ?></div>
	...
	function box_meta( $args = null ) {
	    ...
	    $meta_icon = isset( $args['meta_icon'] ) ? $args['meta_icon'] : 'globe';
	    ...
	    if ( empty( $meta_value ) ) {
	        ...
	        $meta_html = '<i class="fa fa-' . $meta_icon .'" aria-hidden="true"></i>' . $meta_value;
	    }
