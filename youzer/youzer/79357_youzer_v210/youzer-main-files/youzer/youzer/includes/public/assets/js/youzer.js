( function( $ ) {
	...
	$( document ).ready( function() {
		...
		$( window ).load( function() {
			...
			// Remove Select2 Plugin Inputs.
			if ( $( '.youzer .select2' )[0] ) {
				$( '.youzer .select2' ).remove();
			}
		 ...
		 */
		$( document ).on( 'click',  '.yz-show-activity-tools', function ( e ) {

			// Switch Current Icon.
			$( this ).toggleClass( 'yz-close-activiy-tools' );

			// Show / Hide Activity Tools.
			$( this ).closest( '.activity-item' ).find( '.yz-activity-tools' ).fadeToggle();
	    ...
	    $.yz_DialogMsg = function ( type, msg ) {
			...
			if ( type == 'error' ) {
	     	...
	     	// Get Header Icon.
	     	if ( type == 'error' ) {
	     		dialogHeader = '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i>';
	     	} else if ( type == 'info' ) {
	     		dialogHeader = '<i class="fa fa-info-circle" aria-hidden="true"></i>';
	     	} else if ( type == 'success' ) {
	     		dialogHeader = '<i class="fa fa-check" aria-hidden="true"></i>';
	     	}
	     	...
	     	'<div class="yz-' + type + '-dialog">' +
	            '<div class="youzer-dialog-container">' +
	                ...
	                '<div class="youzer-dialog-msg">' +
	                    '<div class="youzer-dialog-desc">' + dialogTitle + msg + '</div>' +
	               	'</div>' +
	    ...
	    .append( '<i class="fa fa-times yz-close-login" aria-hidden="true"></i>' );
	    ...
	    $( '.logy-login-form' ).on( 'submit', function( e ) {
	    	...
	    	yz_submit_btn.find( '.logy-button-icon i' ).attr( 'class', 'fa fa-spinner fa-spin' );
	        ...
	        $.ajax({
	            ...
	            success: function( response ) {
	                ...
	                if ( response.loggedin == true ) {
	    				...
	    				yz_submit_btn.find( '.logy-button-icon i' ).attr( 'class', 'fa fa-check' );
		         		// Redirect.
