...
class YZ_Video {
    ...
    function settings() {
        ...
        $supported_videos = "https://en.support.wordpress.com/videos/";

        // Filter Url
        $supported_videos = apply_filters( 'yz_account_supported_videos_url', $supported_videos );

        $Yz_Settings->get_field(
            array(
                'title' => __( 'video url', 'youzer' ),
                'desc'  => sprintf( __( "here's the <a href='%s' target='_blank'>list of supported websites</a>", 'youzer' ), $supported_videos ),
                'id'    => 'wg_video_url',
                'type'  => 'text'
            ), true
        );
