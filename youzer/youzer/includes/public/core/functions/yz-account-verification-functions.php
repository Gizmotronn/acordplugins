...
function yz_get_user_verification_tool( $user_id = null, $icons = null ) {
	...
	// Get User Verification Action.
	$action = $is_account_verified ? 'unverify' : 'verify';
	
	// Get Button Title.
	$button_icon = $is_account_verified ? 'times' : 'check';
	
	// Get Button Title.
	$button_title = $is_account_verified ? __( 'Unverify Account', 'youzer' ) : __( 'Verify Account', 'youzer' );
	
	// Get Ajax Nonce
	$ajax_nonce = wp_create_nonce( 'yz-account-verification-' . $user_id );

	?>

	<div class="yz-tool-btn yz-verify-btn" <?php if ( 'only-icons' == $icons ) { ?> data-yztooltip="<?php echo $button_title; ?>"<?php } ?> data-nonce="<?php echo $ajax_nonce; ?>" data-action="<?php echo $action; ?>" data-user-id="<?php echo $user_id; ?>">
			<div class="yz-tool-icon"><i class="fa fa-<?php echo $button_icon; ?>"></i></div><?php if ( 'full-btns' == $icons ) : ?><div class="yz-tool-name"><?php echo $button_title; ?></div><?php endif; ?>
	</div>
...
function yz_account_verified_button( $size = null ) {
	...
	// Add Icon Class.
	$class[] = 'fa fa-check yz-account-verified';
