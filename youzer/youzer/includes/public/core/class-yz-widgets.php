...
class Youzer_Widgets {
				...
				<div class="yz-widget-head">
					<h2 class="yz-widget-title">
						<?php if ( 'on' == $display_icon ) : ?>
							<i class="fa fa-<?php echo $widget_icon; ?>" aria-hidden="true"></i>
						<?php endif; ?>
	...
	function get_settings_widgets() {
		...
		// Widgets Without settings list.
		$widgets_without_settings = array(
			'friends', 'groups', 'social_networks', 'recent_posts', 'phone', 'address', 'email', 'website', 'user_tags', 'user_balance', 'user_badges'
		);
		
		// Unset Widgets that have no settings.
		foreach ( $widgets_without_settings as $widget ) {
