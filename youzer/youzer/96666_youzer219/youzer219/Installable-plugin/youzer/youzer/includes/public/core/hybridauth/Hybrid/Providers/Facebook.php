...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model {
    ...
    function loginFinish() {
        ...
        try {
            ...
            throw new YZHybrid_Exception('Facebook Graph returned an error: ' . $e->getMessage());
        } catch (Facebook\Exceptions\FacebookSDKException $e) {
            throw new YZHybrid_Exception('Facebook SDK returned an error: ' . $e->getMessage());
        }

        if (!isset($accessToken)) {
            if ($helper->getError()) {
                throw new YZHybrid_Exception(sprintf("Could not authorize user, reason: %s (%d)", $helper->getErrorDescription(), $helper->getErrorCode()));
            } else {
                throw new YZHybrid_Exception("Could not authorize user. Bad request");
            }
        }
        ...
        try {
            ...
            throw new YZHybrid_Exception($e->getMessage(), 0, $e);
        }
   ...
   function getUserPages($writableonly = false) {
    ...
    function getUserContacts() {
        ...
        while ($pagedList) {
            try {
                ...
                throw new YZHybrid_Exception("User contacts request failed! {$this->providerId} returned an error {$e->getMessage()}", 0, $e);
            }
    ...
    function getUserActivity($stream = 'timeline') {
        try {
            ...
            throw new YZHybrid_Exception("User activity stream request failed! {$this->providerId} returned an error: {$e->getMessage()}", 0, $e);
        }
