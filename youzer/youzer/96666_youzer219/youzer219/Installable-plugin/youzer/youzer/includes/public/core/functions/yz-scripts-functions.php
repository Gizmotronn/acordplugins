...
function yz_public_scripts() {
    ...
    // Headers Css
    wp_enqueue_style( 'yz-headers', YZ_PA . 'css/yz-headers.min.css', array( 'yz-lato', 'yz-roboto' ), $Youzer->version );
...
function yz_call_activity_scripts() {
    ...
    if (
        yz_is_activity_component()
        || 
        ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'youzer_activity') ) 
    ) {
        yz_activity_scripts();
