...
function yz_get_wc_account_menu_items() {
	...
	$wc_items = $new_items + wc_get_account_menu_items();         

	// Add Payment Methods.
	$wc_items['payment-methods'] = __( 'Payment Methods', 'youzer' );

	// Remove Some items.
	unset( $wc_items['dashboard'], $wc_items['customer-logout'] );
...
function yz_woocommerce_sub_tabs() {
	...
	$position = 0;
    // print_r($wc_menu_items);
    foreach ( $wc_menu_items as $key => $item_title ) {
        $position += 10;

        // if ( $key == 'dashboard' ) $key = 'cart';
	...
	// $tabs0 = array(
	// // 'cart' 		=> array( 'slug' => 'cart', 'name' => __( 'Shopping Cart', 'youzer' ), 'position' => 10 ),
	// // 'checkout' 	=> array( 'slug' => 'checkout', 'name' => __( 'Checkout', 'youzer' ), 'position' => 20 ),
	// 'orders' 	=> array( 'slug' => 'orders', 'name' => __( 'Orders', 'youzer' ), 'position' => 30 ),
	// 'track' 	=> array( 'slug' => 'track', 'name' => __( 'Track', 'youzer' ), 'position' => 40 ),
	// 'downloads' => array( 'slug' => 'downloads', 'name' => __( 'Downloads', 'youzer' ), 'position' => 50 ),
	// 'edit-address' => array( 'slug' => 'edit-address', 'name' => __( 'Addresses', 'youzer' ), 'position' => 60 ),
	// // 'payment-methods' => array( 'slug' => 'payment-methods', 'name' => __( 'Payment Methods', 'youzer' ), 'position' => 70 ),
	// );
...
function yz_woocommerce_scripts() {
    ...
    // Call Script.
    wp_enqueue_style( 'yz-woocommece', YZ_PA . 'css/yz-woocommerce.min.css' );
...
function yz_is_cart_page() {
	...
	if ( is_user_logged_in() && ! defined( 'WOOCOMMERCE_CART' ) && yz_wc_is_sub_tab_exist( 'cart' ) && yz_is_woocommerce_tab( 'cart') ) {
 ...
 */
function yz_wc_available_payment_gateways( $available_gateways ) {
...
function yz_is_checkout_page() {
	...
	if ( is_user_logged_in() && ! defined( 'WOOCOMMERCE_CHECKOUT' ) && yz_wc_is_sub_tab_exist( 'checkout' ) && yz_is_woocommerce_tab( 'checkout' ) ) {
