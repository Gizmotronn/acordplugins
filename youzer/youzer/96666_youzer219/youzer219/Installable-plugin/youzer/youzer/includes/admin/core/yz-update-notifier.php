...
define( 'YOUZER_PLUGIN_NOTIFIER_CACHE_INTERVAL', 1 );
...
function YOUZER_update_plugin_notifier_menu() {  
	if ( function_exists( 'simplexml_load_string' ) ) { // Stop if simplexml_load_string funtion isn't available
	    $xml 			= YOUZER_get_latest_plugin_version( YOUZER_PLUGIN_NOTIFIER_CACHE_INTERVAL ); // Get the latest remote XML file on our server
		$plugin_data 	= get_plugin_data( WP_PLUGIN_DIR . '/' . YOUZER_NOTIFIER_PLUGIN_FOLDER_NAME . '/' . YOUZER_NOTIFIER_PLUGIN_FILE_NAME ); // Read plugin current version from the style.css

		if ( (string) $xml->latest > (string) $plugin_data['Version'] ) { // Compare current plugin version with the remote XML version
			if ( defined( 'YOUZER_NOTIFIER_PLUGIN_SHORT_NAME' ) ) {
				$menu_name = YOUZER_NOTIFIER_PLUGIN_SHORT_NAME;
			} else {
				$menu_name = YOUZER_NOTIFIER_PLUGIN_NAME;
			}
...
function YOUZER_update_notifier_bar_menu() {
	if ( function_exists( 'simplexml_load_string' ) ) { // Stop if simplexml_load_string funtion isn't available
		...
		$xml 		= YOUZER_get_latest_plugin_version( YOUZER_PLUGIN_NOTIFIER_CACHE_INTERVAL ); // Get the latest remote XML file on our server
		$plugin_data 	= get_plugin_data( WP_PLUGIN_DIR . '/' . YOUZER_NOTIFIER_PLUGIN_FOLDER_NAME . '/' .YOUZER_NOTIFIER_PLUGIN_FILE_NAME ); // Read plugin current version from the main plugin file
...
function YOUZER_update_notifier() { 
	...
	<div class="wrap">
	    ...
	    <div id="message" class="updated below-h2"><p><strong>There is a new version of the <?php echo YOUZER_NOTIFIER_PLUGIN_NAME; ?> plugin available.</strong> You have version <?php echo $plugin_data['Version']; ?> installed. Update to version <?php echo $xml->latest; ?>.</p></div>
...
function YOUZER_get_latest_plugin_version( $interval ) {
