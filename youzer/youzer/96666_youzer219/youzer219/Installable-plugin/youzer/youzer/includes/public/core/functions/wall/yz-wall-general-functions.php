 ...
 */
function yz_wall_hide_private_users_posts( $where ) {
...
add_filter( 'bp_activity_get_where_conditions', 'yz_wall_hide_private_users_posts', 10 );

/**
 * Get Wall Activities By User Role.
 */
function yzc_get_activities_by_ro0le( $where ) {
    ...
    // $args = array( 'role' => 'subscriber', 'fields' => 'ID' );
...
// add_filter( 'bp_activity_get_where_conditions', 'yzc_get_activities_by_role', 10 );
...
function yz_temporary_upload_directory( $dir ) {
	global $YZ_upload_folder;
    return array(
        'path'   => $dir['basedir'] . '/' . $YZ_upload_folder . '/temp',
        'url'   => $dir['baseurl'] . '/' . $YZ_upload_folder . '/temp',
